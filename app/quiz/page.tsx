...
export default function Quiz() {
  ...
  return (
    <div className={`flex flex-col min-h-screen px-4 sm:px-6 lg:px-8 py-10 ${pageBg(darkMode)}`}>
      ...
      <div className={`max-w-xl w-full mx-auto p-8 sm:p-10 rounded-2xl shadow-2xl mt-16 border-2 ${
        ...
        <div className="mb-6">
          ...
          <div className={`w-full h-2 rounded-full ${darkMode ? "bg-white/10" : "bg-[#1E3D2A]/10"}`}>
            <div
              className={`h-2 rounded-full transition-all duration-500 ease-out ${darkMode ? "bg-[#65F0CD]" : "bg-[#1E3D2A]"}`}
        ...
        <div className="flex flex-col gap-3">
          {currentQuestion.answers.map((answer) => (
            <button
              ...
              className={`text-left p-3 sm:p-4 border-2 rounded-lg transition-all duration-200 ${
                selectedAnswers[currentQuestion.id] === answer.id
                  ? darkMode
                    ? "bg-[#65F0CD]/30 border-[#65F0CD] text-white"
                    : "bg-[#1E3D2A]/15 border-[#1E3D2A] text-[#1E3D2A] font-semibold"
                  : darkMode
                  ? "bg-white/10 border-white/30 text-white hover:border-[#65F0CD] hover:bg-[#65F0CD]/10"
                  : "bg-white border-[#1E3D2A]/40 text-[#1E3D2A] hover:border-[#1E3D2A] hover:bg-[#1E3D2A]/5"
              }`}
            >
              {answer.text}
            </button>

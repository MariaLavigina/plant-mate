...
import { accentText, primaryButton } from "../lib/styles";
...
export default function DesktopHotspotPopup({ hotspot, onClose, position }) {
  ...
  return (
    ...
    >
      <button
        onClick={onClose}
        className={`absolute -top-2 -right-2 w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center text-xs md:text-sm font-bold transition-all duration-200 hover:scale-110 shadow-lg ${primaryButton(darkMode)}`}
        aria-label="Close popup"
      >
        ×
      </button>

      <h3 className={`text-base md:text-lg lg:text-xl font-bold mb-2 ${accentText(darkMode)}`}>
        {hotspot.title}
      </h3>
